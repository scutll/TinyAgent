...
description = """
请阅读以下用户与 Agent 的对话，并以我（模型）的第一人称视角压缩关键信息。输出需简洁，重点保留：1）用户的核心诉求与限制条件；2）我完成的主要操作及给出的响应；3）最终结论或待办事项。次要细节可概述，不重要的细节可以略过甚至丢弃。4）如果你认为对话的最后一部分包含一段未完成的任务，你需要最大限度的保留当前任务的信息以确保在信息压缩以后还能借助这一段信息完成任务
...
示例结果: 我阅读了用户的文档，用户想要我帮他完成他的Java作业，为用户生成了对应的代码并将文件创建在document/文件夹里面，但我还差一段对代码的使用说明还未完成，我可能需要重新阅读这段代码来编写代码使用说明
---
\n\n\n
"""
